...
flowchart TB
    ...
    style DINGTALK fill:#fff3e0,stroke:#f57c00
...
| 层次 | 组件 | 说明 |
|------|------|------|
| **统一入口** | 钉钉工作台 | 用户通过钉钉访问所有功能，单点登录 |
| **流程引擎** | 钉钉宜搭 + 审批流 | 合同审批流程由钉钉提供，无需自建 |
| **消息通知** | 钉钉 IM | 审查结果、待办任务通过钉钉消息推送 |
| **任务中心** | 钉钉待办 | 待审查合同、待审批事项统一在待办中呈现 |
| **应用服务** | 阿里云 ECS | 核心业务服务部署在云服务器上 |
| **文件存储** | 阿里云 OSS | 合同原文、审查报告等文件对象存储 |
| **数据库** | 阿里云 RDS | 结构化数据持久化存储 |
| **缓存** | 阿里云 Redis | 热点数据缓存，提升响应速度 |
| **AI 推理** | 阿里云函数计算 | 文档解析、风险识别等 AI 能力按需调用 |

#### 钉钉集成能力

| 钉钉能力 | 集成方式 | 用途 |
|----------|----------|------|
| **工作台入口** | H5 微应用 / 宜搭页面 | 用户统一访问入口 |
| **审批流** | 审批引擎 API | 合同审批流程配置与执行 |
| **消息通知** | 消息接口 | 审查完成、待办提醒等消息推送 |
| **待办任务** | 待办接口 | 将待审查/待审批任务推送到用户待办 |
| **IM 能力** | 群机器人 | 审查结果群通知、异常预警 |
| **在线预览** | 文档预览组件 | 合同原文在线查看 |
| **身份认证** | OAuth / 免登 | 统一身份认证，无需单独登录 |

#### 技术优势

- **入口统一**：用户日常工作在钉钉进行，无需切换系统，使用门槛低
- **流程复用**：直接使用钉钉成熟的审批能力，无需自建流程引擎
- **弹性扩展**：基于阿里云 Serverless 架构，AI 能力按需调用，成本可控
- **运维简化**：基础设施托管，聚焦业务逻辑开发
